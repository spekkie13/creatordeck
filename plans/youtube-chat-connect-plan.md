...
- Spec §4 mentions channel **avatar** on the Connections page; the repo shows name/ID only and stores no thumbnail URL (owner question §4 Q4).
...
2. **Drizzle migration #1** — in `src/lib/schema.ts`, extend `linkedAccounts`: `tokenExpiresAt: timestamp("token_expires_at")`, `scopes: text("scopes")`. Run `npm run db:generate` → `drizzle/0001_*.sql`, then `npm run db:push:all`. (Token encryption is a value-format change, not a schema change — the `text` columns stay.)
...
4. **Connect flow completion** — `connectionsService.linkGoogleAccount()`: persist `tokenExpiresAt` (`now + expires_in`), granted `scope` string; keep PKCE/state as-is (already solid).
...
7. **Connections UI** — keep Connect/Manage components; update manage copy from "Live chat poller" to broadcast-status language; add reconnect state for broken tokens.
...
## 4. Open questions for the owner (product-level only)

1. **Memberships:** spec parks them, but membership events arrive inside the same `liveChatMessages` responses (either ingestion mode) at zero extra quota, and the repo already has `yt_member_events`, member goals, and member UI (currently orphaned once the scraper dies). Keep them dark in v1 per spec, or ingest them since the pipe is free? (Plan assumes: dark in v1.)
2. **Google as a login method:** "Continue with YouTube" login keeps the sensitive `youtube.readonly` scope attached to *sign-in* (it must resolve a channelId to key the account). Keep Google login, or make Twitch the sole login and YouTube strictly a connection? The latter simplifies the Google verification story to one flow. (Plan assumes: keep login, readonly scope.)
3. **Encrypt Twitch/Spotify tokens too:** the encryption helper covers the shared `linked_accounts` columns; encrypting all providers on next write is nearly free. In scope now, or Google-only per spec letter? (Plan assumes: helper applies to all providers on write; no backfill task.)
4. **Channel avatar on Connections page:** spec §4 mentions name/avatar; repo stores no thumbnail. Add an `avatar_url` column + fetch at connect time, or ship name-only in v1? (Plan assumes: name-only; trivial to add later.)
